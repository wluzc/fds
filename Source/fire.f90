   ...
   IF (SUPPRESSION) THEN
      DO NR=1,N_REACTIONS
         ...
         IF (ZZ_0(RN%FUEL_SMIX_INDEX)>ZZ_MIN_GLOBAL .AND. ZZ_0(RN%AIR_SMIX_INDEX)>ZZ_MIN_GLOBAL) THEN
            CALL CHECK_AUTO_IGNITION(EXTINCT,TMP_IN,RN%AUTO_IGNITION_TEMPERATURE,IIC,JJC,KKC,NR,K_SGS,DELTA,MU_IN,RHO_IN)
            IF (.NOT.EXTINCT) EXIT
         ENDIF
      ENDDO
   ENDIF
...
SUBROUTINE CHECK_AUTO_IGNITION(EXTINCT,TMP_IN,AIT,IIC,JJC,KKC,REAC_INDEX,K_SGS,DELTA,MU_IN,RHO_IN)
...
REAL(EB), INTENT(IN) :: TMP_IN,AIT,K_SGS,DELTA,MU_IN,RHO_IN
...
ELSE
   S_SGS = 0._EB
ENDIF
!WRITE(LU_ERR,*) RN%A_COEF,RN%B_COEF,RN%C_COEF,RN%D_COEF
T_IGN_MOD = RN%A_COEF*S_SGS**3 + RN%B_COEF*S_SGS**2 + RN%C_COEF*S_SGS + RN%D_COEF
!WRITE(LU_ERR,*) MU_IN,RHO_IN, DELTA, K_SGS, S_SGS
!WRITE(LU_ERR,*) T_IGN_MOD
...
USE PHYSICAL_FUNCTIONS, ONLY: GET_MASS_FRACTION
!USE SPECIES, ONLY: SPECIES_MIXTURE, N_SPECIES

REAL(EB),INTENT(IN) :: TMP_IN,TAU_MIX,ZZ_IN(1:N_TRACKED_SPECIES),ZZ_0(1:N_TRACKED_SPECIES)
LOGICAL, INTENT(INOUT) :: EXTINCT
REAL(EB) :: ZZ_HAT_0(1:N_TRACKED_SPECIES),ZZ_HAT(1:N_TRACKED_SPECIES),H_0,H_CRIT,PHI_TILDE,CFT
INTEGER :: NS,NR,NL !added NL for lumped species loop
REAL(EB) :: SUM_ZZ
TYPE(REACTION_TYPE), POINTER :: RN,R1

!New variables for mass fraction computation
INTEGER, SAVE :: NS_N2 = -2, NS_CO2 = -2, NS_H2O = -2
REAL(EB) :: Y_N2_HAT,Y_CO2_HAT,Y_H2O_HAT
REAL(EB) :: T_CFT_N2_CO2, T_CFT_H2O
INTEGER :: NN
INTEGER, SAVE :: CO2_INDEX = -1, N2_INDEX = -1, H2O_INDEX = -1

! find indices for N2, CO2, and H2O 

IF (CO2_INDEX < 0) THEN
   
!   WRITE(LU_ERR,*) N2_INDEX, CO2_INDEX, H2O_INDEX
      
   DO NN=1, N_SPECIES
      IF (TRIM(SPECIES_MIXTURE(NN)%ID) == 'NITROGEN')         N2_INDEX  = NN
      IF (TRIM(SPECIES_MIXTURE(NN)%ID) == 'CARBON DIOXIDE')   CO2_INDEX = NN
      IF (TRIM(SPECIES_MIXTURE(NN)%ID) == 'WATER VAPOR')      H2O_INDEX = NN
!      WRITE(LU_ERR,*) TRIM(SPECIES_MIXTURE(NN)%ID)
   ENDDO
   
ENDIF


! initialise mass fractions
Y_N2_HAT  = 0._EB
Y_CO2_HAT = 0._EB
Y_H2O_HAT = 0._EB
...
! compute mass fractions of N2, CO2, and H2O

IF (N2_INDEX > 0) THEN
   CALL GET_MASS_FRACTION(ZZ_HAT,N2_INDEX,Y_N2_HAT)  
ELSE 
   Y_N2_HAT  = 0._EB
ENDIF

IF (CO2_INDEX > 0) THEN
   CALL GET_MASS_FRACTION(ZZ_HAT,CO2_INDEX,Y_CO2_HAT) 
ELSE 
   Y_CO2_HAT = 0._EB
ENDIF
...
IF (H2O_INDEX > 0) THEN
   CALL GET_MASS_FRACTION(ZZ_HAT,H2O_INDEX,Y_H2O_HAT) 
ELSE 
   Y_H2O_HAT = 0._EB
ENDIF

!WRITE(LU_ERR,*) Y_N2_HAT,Y_CO2_HAT,Y_H2O_HAT

! compute critical flame temperature following Eq. (24) of Maragkos et al.

T_CFT_N2_CO2 = 1407.4_EB * ((1000.0_EB*TAU_MIX)**(-0.073_EB))
T_CFT_H2O    = 1517.8_EB * ((1000.0_EB*TAU_MIX)**(-0.034_EB))

CFT = (Y_N2_HAT + Y_CO2_HAT) * T_CFT_N2_CO2 + Y_H2O_HAT * T_CFT_H2O

!WRITE(LU_ERR,*) (Y_N2_HAT + Y_CO2_HAT), T_CFT_N2_CO2, Y_H2O_HAT, T_CFT_H2O, CFT
